  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-zinc-400">Track your manufacturing orders</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <Package className="mb-4 h-16 w-16 text-zinc-700" />
          <h2 className="mb-2 text-xl font-semibold text-white">No orders yet</h2>
          <p className="text-zinc-400">Accept a quote to create your first order</p>
        </div>
      ) : (
        <div className="space-y-4">
          {orders.map((order) => {
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-xl font-bold text-white">${order.total_amount?.toLocaleString()}</p>
                    ...
                    {order.timeline.map((event, i) => (
                      <span key={i} className="flex items-center gap-1">
                        ...
                        {i < order.timeline.length - 1 && (
                          <span className="mx-1 h-px w-4 bg-zinc-700" />
                        )}
                      </span>
                    ))}
                  </div>
